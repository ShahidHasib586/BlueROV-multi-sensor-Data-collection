...
# Use python3 directly for multivideo (instead of ros2 run)
MULTIVIDEO_PY = os.path.join(WORKSPACE_DIR, "autonomous_rov", "autonomous_rov", "multivideo.py")
...
COMMANDS = {
    ...
    # CHANGED HERE ONLY: run multivideo via python3
    "Multivideo (3 cams)": f"python3 '{MULTIVIDEO_PY}'",
...
def bash_command(cmd: str) -> str:
    ...
    # If multivideo.py path is used, ensure it exists
    if "multivideo.py" in cmd and not os.path.exists(MULTIVIDEO_PY):
        raise FileNotFoundError(
            f"multivideo.py not found at:\n{MULTIVIDEO_PY}\n\n"
            f"Expected source path inside your workspace."
        )

    # Reduce FastDDS SHM noise
    env = "export FASTDDS_SHM_TRANSPORT_DISABLED=1;"

    # Make GUI apps more reliable when launched from Tk
    # Keep existing behavior, just ensuring DISPLAY/XAUTHORITY are not lost.
    env += "export DISPLAY=${DISPLAY:-:0};"
